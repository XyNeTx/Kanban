class labDate {
    ...
    Date(pFormat = 'yyyy-MM-dd', pFunction = '') {
        ...
        if (pFunction != '') {
            ...
            if (_arr[0].toUpperCase() === 'DD') _now = new Date(_now.getFullYear(), _now.getMonth(), eval(parseInt(_now.getDate()) + parseInt(_arr[1])));
            if (_arr[0].toUpperCase() === 'MM') _now = new Date(_now.getFullYear(), eval(parseInt(_now.getMonth()) + parseInt(_arr[1])), _now.getDate());
            if (_arr[0].toUpperCase() === 'YY' || _arr[0].toUpperCase() === 'YYYY') _now = new Date(eval(parseInt(_now.getFullYear()) + parseInt(_arr[1])), _now.getMonth(), _now.getDate());
            //console.log(_now);
        }
    ...
    Format = function (pDate = '', pFormat = 'yyyyMMdd', pFunction = '') {
        var _yy = '', _mm = '', _dd = '';
        if (pDate.length == 8) {
        ...
        if (pFunction != '') {
            let _arr = pFunction.split("=");
            if (_arr[0].toUpperCase() === 'DD') pDate = new Date(_yy, _mm, eval(parseInt(_dd) + parseInt(_arr[1])));
            if (_arr[0].toUpperCase() === 'MM') pDate = new Date(_yy, eval(parseInt(_mm) + parseInt(_arr[1])), _dd);
            if (_arr[0].toUpperCase() === 'YY' || _arr[0].toUpperCase() === 'YYYY') pDate = new Date(eval(parseInt(_yy) + parseInt(_arr[1])), _mm, _dd);
        }
