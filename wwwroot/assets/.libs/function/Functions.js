...
function MaskCurrency(value, decimal = 2, showZero = false, keep=false) {
    ...
    if (value != '') {
        if (value.indexOf('.') >= 0) {
            ...
            if (_length < decimal) value = parseFloat(value + '0');
            if (value == 0) value = (showZero ? '0.00' : '');
            if (!keep) {
                if (value != 0) value = value.toFixed(decimal).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
            } {
                value = parseFloat(_digit);
                if (value >= 0) value = value.toFixed(0).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,") + '.' + (_length < decimal ? _decimal + '0' : _decimal);
            }
...
function RadioValue(pObject) {
    ...
    $('input[name=' + pObject + ']').each(function (index, obj) {
        if ($('#' + obj.id).prop('checked') == true) _ret = $('#' + obj.id).val();
    });
