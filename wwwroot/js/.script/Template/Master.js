    ...
    constructor(pConfig = '') {
        this.prepare = function () {
            ...
            if (_PERMISSION_.new) {
                ...
                if (typeof (pCallback) === 'function' && this.edit() == undefined)
                    _addnew = false;
            }
        ...
        this.initial = function (pCallback = '') {
        ...
        this.search = function (pCallback = '') {
            const _Controller = this.Controller;
        ...
        this.editCallback = function (row) {
            if ($.isFunction(window.onEdit))
                onEdit(row);
        };
        this.edit = function (row = null) {
            ...
            $('#' + _FormName + ' .gj-icon').each(function () {
                if ($(this).parent().prev()[0].hasAttribute('noedit'))
                    $(this).parent().removeAttr('readonly');
                $(this).unbind('click');
            });
            ...
            $('#' + _FormName + ' input').each(function () {
                if (this.id != '') {
                    if ($('#' + this.id).attr('type') == 'color') {
                        ...
                        if (row != null)
                            if (row[this.id] == 1 || row[this.id] == true || row[this.id] == 'true')
                        ...
                        if (document.getElementById(this.id).hasAttribute('noedit'))
                            $('#' + _FormName + ' #' + this.id).removeAttr('readonly');
                    }
                }
            });
            $('#' + _FormName + ' select').each(function () {
                if (this.id != '') {
                    ...
                    if ($('#' + _FormName + ' #' + this.id)[0].hasAttribute('noedit'))
                        $('#' + _FormName + ' #' + this.id).removeAttr('readonly');
                }
            });
            if (row == null) {
                ...
                $.each(row, function (k, v) {
                    ...
                    if ($('#' + _FormName + ' #' + k)[0] != undefined)
                        if ($('#' + _FormName + ' #' + k)[0].hasAttribute('noedit'))
                            $('#' + _FormName + ' #' + k).attr('readonly', 'readonly');
                });
                //for disable datetime picker component
                $('#' + _FormName + ' .gj-icon').each(function () {
                    if ($(this).parent().prev()[0].hasAttribute('noedit'))
        ...
        this.save = function (pCallback = '') {
            ...
            xSwal.questionPack(i18nLayout.modal.button.swal_save, function (result) {
        ...
        this.delete = function (pCallback = '') {
            const _Controller = this.Controller;
            const _ModalName = this.ModalName;
            const _FormName = this.FormName;
            xSwal.questionPack(i18nLayout.modal.button.swal_delete, function (result) {
                if (result.isConfirmed) {
                    xSplash.show();
        ...
        this.deleteall = function () {
            return __awaiter(this, arguments, void 0, function* (pCallback = '') {
                xSwal.questionPack(i18nLayout.modal.button.swal_deleteall, function (result) {
