...
$(document).ready(function () {
    ...
    initial = function () {
        if (getCookie('ProcessYM') != '')
            $('#txtProcessYM').val(getCookie('ProcessYM'));
        ...
        xAjax.Post({
            ...
            then: function (result) {
                ...
                if (result.response == 'OK') {
                    ...
                    if (result.data.ReceiveDatePCW.length > 0)
                        $('#tblReceiveDatePCW').dataTable().fnAddData(result.data.ReceiveDatePCW);
