...
$(document).ready(function () {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield _xLib.AJAX_Get('/api/KBNOR121/OnLoad', { Login_Date: _CookieLoginDate, Shift: shift, Process_Date: $("#inputProcessDateFor").val() });
        yield _xLib.AJAX_Get('/api/KBNOR121/SupplierDropDown', '', function (result) {
        ...
        yield _xLib.AJAX_Post('/api/KBNOR121/Chk_Status_CCR', JSON.stringify(obj), function (success) {
...
$("#btnEditNews").click(function () {
    ...
    console.log(obj);
    _xLib.AJAX_Post('/api/KBNOR121/UpdateInformNews', JSON.stringify(obj), function (result) {
...
$("#btnNextPage").click(function () {
    return __awaiter(this, void 0, void 0, function* () {
        var page = parseInt($("#txtPage").val().split("/")[0]);
        if (page == parseInt($("#txtPage").val().split("/")[1]))
            return;
        $("#txtPage").val((parseInt(page) + 1) + "/" + dT_PartControl.length);
        previewFunction($("#txtPage").val().split("/")[0] - 1, _command); // -1 because index start at 0
        window.scrollTo(0, 0);
...
const previewFunction = (intRow, _command) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield _xLib.AJAX_Post('/api/KBNOR121/Get_All_Data', JSON.stringify(obj), function (result) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ($("#readSupplier").val() != dT_PartControl[0].F_Supplier_Code + "-" + dT_PartControl[0].F_Supplier_Plant) {
                ...
                $("#txtPage").val(`${intRow + 1}/${dT_PartControl.length}`);
            }
            ...
            dT_Period.forEach(function (item) {
                ...
                if (dateSet.some(f => f.includes(itemDate))) {
                    ...
                    if (_dayColSpan != 0)
                        THeadR2.append(`<th style="border:0px;background-color:#F65900" colspan="${_dayColSpan}" class="text-light">Day</th>`);
                    if (item.F_Period != 0)
                        THeadR2.append(`<th style="border:0px" colspan="${item.F_Period}" class="bg-primary text-light">Night</th>`);
                    count = 1; // Reset count Use Count for assign T + Count
                    for (let i = 0; i < _dayColSpan; i++) {
                        if (i == 0) {
                            THeadR3.append(`<th id='1stTDay'>T${count}</th>`);
                        }
                        else {
                            THeadR3.append(`<th id='TDay${count}'>T${count}</th>`);
                        }
                        count++;
                    }
                    for (let i = 0; i < item.F_Period; i++) {
                        if (i == 0) {
                            THeadR3.append(`<th id='1stTNight'>T${count}</th>`);
                        }
                        else {
                            THeadR3.append(`<th id='TNight${count}'>T${count}</th>`);
                        }
                        count++;
                    }
                    // reset dayColSpan
            ...
            yield addDetailToTable(dateSet, intRow);
            yield sumKB(dateSet);
...
const addDetailToTable = (dateSet, intRow) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let i = 0; i < _headLength; i++) {
        ...
        //console.log(_headerQty + " HeaderQty");
        if (_headerQty == undefined)
            break; //if out of index then break loop
        ...
        if (!dateSet.some(f => f.includes(_headerDate)))
            continue; //if date not in dateSet then skip loop
        ...
        for (let k = 1; k <= 20; k++) {
            if (k >= 15 || k == 12)
                $(`#TBodyR${k}`).append(`<td id=tdR${k}Pcs${dateSet[_countDateSet]}></td>`);
            else
                $(`#TBodyR${k}`).append(`<td id=tdR${k}Pcs${dateSet[_countDateSet]}>${_headerQty[_setAccessHeader[k - 1]]}</td>`);
        }
        for (let j = 0; j <= _headColSpan - 2; j++) {
            ...
            for (let k = 1; k <= 20; k++) {
                ...
                if (k == 12 && _id == "KB") {
                    ...
                    if (isNaN(_headerQty["F_KB_CutAdd"]) || _headerQty["F_KB_CutAdd"] === Infinity)
                        _headerQty["F_KB_CutAdd"] = 0;
        ...
        yield _xLib.AJAX_Post('/api/KBNOR121/Get_BL', JSON.stringify(obj), function (success) {
            return __awaiter(this, void 0, void 0, function* () {
                if (success.status == 200) {
                    success = yield _xLib.JSONparseAndTrim(success);
                    //console.log(success.data);
        ...
        _countDateSet += 1;
        if (_countDateSet >= dateSet.length)
            _countDateSet = 0;
    }
    let _countT = 1;
    ...
    dT_Detail.filter(x => x.F_Part_No + "-" + x.F_Ruibetsu == $("#readPartNo").val()
        ...
        && x.F_Store_Code == $("#readStoreCode").val()
        && x.F_Kanban_No == $("#readKanbanNo").val()).forEach(function (item, i) {
        //console.log(item);
        ...
        for (let k = 1; k <= 20; k++) {
            ...
            if (k == 11 || k == 12 || k == 13) { //convert qty to KB fixed by row
                ...
                $(`#${_insIdDetail}`).text(item[_setAccessDetail[k - 1]]);
    ...
    // ------------------------------------ DT AdjustOrder_Trip -----------------------------------------
    // ------------------------------------ DT Volume -----------------------------------------
    yield dT_Volume.filter(x => x.F_Part_No + "-" + x.F_Ruibetsu == $("#readPartNo").val()
        && x.F_Supplier_Code == $("#inputSupplier").val().split("-")[0]
        && x.F_Supplier_Plant == $("#inputSupplier").val().split("-")[1]
        && x.F_Store_Code == $("#readStoreCode").val()
        && x.F_Kanban_No == $("#readKanbanNo").val()).forEach(function (item, i) {
        ...
        _Row.forEach(function (row, i) {
            ...
            if (item[_Key[i]] == undefined || item[_Key[i]] == 0 || item[_Key[i]] == 0)
                return;
            ...
            if (isNaN(_PatternKB) || _PatternKB == Infinity)
                _PatternKB = 0;
    ...
    dT_Actual_Receive.filter(x => x.F_PART_No + "-" + x.F_RUibetsu == $("#readPartNo").val()
        && x.F_Supplier_code == $("#inputSupplier").val().split("-")[0]
        && x.F_Supplier_Plant == $("#inputSupplier").val().split("-")[1]
        && x.F_Store_Cd == $("#readStoreCode").val()).forEach(function (item, i) {
    ...
    // ------------------------------------ DT Actual -----------------------------------------
    yield _xLib.AJAX_Get('/api/KBNOR121/Detail_Data', { intRow: $("#txtPage").val().split("/")[0] - 1, F_Supplier_Cd: $("#inputSupplier").val() }, function (success) {
        if (success.status == 200) {
            success.data = _xLib.JSONparseAndTrim(success.data);
            console.log(success.data);
    ...
    let _tableShift = "";
    $(`table tbody tr td:not([id*='Pcs']):not([id*='KB'])`).each(function () {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    // Set Style for Table
    $("table tbody tr td[id*='Pcs'],table tbody tr td[id*='KB']").each(function () {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    for (let i = 0; i < _Row.length; i++) {
        ...
        //console.log(_Row[i]);
            ...
            $(`table tbody tr td[id*='tdR${_Row[i]}T']`).each(function () {
                ...
                else if (_bl < _intMinStock)
                    $(this).addClass("bg-danger");
                else if (_bl <= _intStockB && _bl >= _intMinStock)
...
const periodFilter = () => __awaiter(void 0, void 0, void 0, function* () {
    if (_command == "Preview")
        return;
    let arr = [];
    for (let i = 0; i < dT_DeliveryDate.length; i++) {
        ...
        if (_CookieLoginDate.slice(10, 11) == "N")
            trip = parseInt(dT_Period[0]["F_Period"]) + parseInt(dT_Period[1]["F_Period"]);
        if (arr.some(f => f.deliveryDate == deliveryDate && f.deliveryTrip == deliveryTrip))
            continue;
...
const sumKB = (dateSet) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    dT_AdjustOrder_Trip.filter(x => x.F_Part_No + "-" + x.F_Ruibetsu == $("#readPartNo").val()
        ...
        && x.F_Store_Code == $("#readStoreCode").val()
        && x.F_Kanban_No == $("#readKanbanNo").val()
        && x.F_Adj_Pattern != 0).forEach(function (item, i) {
...
$("#btnBlCal").click(function () {
    ...
    _xLib.AJAX_Post('/api/KBNOR121/Bl_Recalculate', JSON.stringify(obj), function (result) {
        if (result.status == 200) {
            xSplash.hide();
...
$("#btnReCal").click(function () {
    ...
    _xLib.AJAX_Post('/api/KBNOR121/Recalculate', JSON.stringify(obj), function (result) {
        if (result.status == 200) {
            xSplash.hide();
