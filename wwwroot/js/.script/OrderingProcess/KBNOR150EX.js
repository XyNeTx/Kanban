...
$(document).ready(function () {
    const KBNOR150EX = new MasterTemplate({
        Controller: _PAGE_,
        Table: 'tblMaster',
        ColumnTitle: {
    ...
    KBNOR150EX.prepare();
    KBNOR150EX.initial(function (result) {
        //console.log(result);
    ...
    xAjax.onClick('btnExportData', function () {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            MsgBox("Do you want to export order data?", MsgBoxStyle.OkCancel, function () {
                return __awaiter(this, void 0, void 0, function* () {
                    xItem.progress({ id: 'prgProcess', current: 5, label: 'Loading data : {{##.##}} %' });
                    var _dt = yield xAjax.ExecuteJSON({
                        data: {
                            "Module": "[exec].[spKBNOR150EX]",
                            "@OrderType": "N",
...
`;
                    if (_dt.rows != null) {
                        for (var i = 0; i < _dt.rows.length; i++) {
                            ...
                            if (i == 0)
                                _text = _header + _text;
                            var _result = yield xAjax.PostAsync({
                                url: "KBNOR150EX/write",
                                data: {
                        ...
                        if (_result.response == 'OK') {
                            MsgBox("Export complete, Do you want to save file? (" + _itmFileName + ")", MsgBoxStyle.OkCancel, function () {
