...
function UploadFile(_files) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const csvFileArrayBuffer = yield csvFile.arrayBuffer();
            const csvFileRead = yield XLSX.read(csvFileArrayBuffer);
...
$("#btnImport").click(function () {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let data = yield UploadFile(_files);
            ...
            for (const each in data) {
                ...
                const _data = yield data[each];
                _data.F_Type = "URGENT";
                let _jsondata = yield JSON.stringify(_data);
                console.log('JSON Data: ', _jsondata);
                yield _xLib.AJAX_Post("/api/KBNIM014/ImportSave", _jsondata, function (response) {
                    //console.log("Success: ", response);
                }, function (err) {
                    return xSwal.error("Error !!", err.responseJSON.message);
                });
            }
            yield _xLib.AJAX_Get("/api/KBNIM014/AfterImported", "", function (response) {
