...
$(document).ready(function () {
    $('[format^="yyyy"][format$="yyyy"]').each(function (e) {
        ...
        var _split = _value;
        if (_value != undefined) {
            if (new Date(_value) == 'Invalid Date')
                _isDate = 1;
            ...
            _value = _format.replace('yyyy', _yy);
        }
        _now = _format.replace('yyyy', _yy);
        ...
        $(this).datepicker({
            ...
            if (typeof window[_onshow] === 'function')
                formok = window[_onshow]();
        }).on("changeDate", function (e) {
            //$(this).val(_value).datepicker('update');
            if (typeof window[_onselect] === 'function')
                formok = window[_onselect](e);
        });
        $(this).val(_value);
    });
    $('[format^="yyyy"][format$="mm"]').each(function (e) {
        ...
        if (_value != undefined) {
            ...
            //_value = (_isDate == 0 ? _default : _format.replace('yyyy', _yy).replace('mm', _mm));
        }
        _now = _format.replace('yyyy', _yy).replace('mm', _mm);
        _value = (_value != undefined ? _value : _now);
        _value = (_default != undefined ? (_default.toUpperCase() == 'NOW' ? _now : _default) : _value);
        $('#' + this.id).attr('style', 'max-width: 20px;');
        //$(this).datepicker({
        //    uiLibrary: 'bootstrap5',
        //    format: _format,
        //    viewMode: "months",
    ...
    $('[format^="yyyy"][format$="dd"]').each(function (e) {
        ...
        var _default = ($(this).attr("default") != undefined ? $(this).attr("default") : 'now');
        var _onshow = ($(this).attr("onshow") != undefined ? $(this).attr("onshow").replace('()', '') : '');
        var _onselect = ($(this).attr("onselect") != undefined ? $(this).attr("onselect").replace('()', '') : '');
        var _now = new Date();
        var _yy = ('20' + _now.getFullYear()).slice(-4);
        ...
        //console.log('x=> ' + _now);
        $(this).off("click");
        if ($('#' + this.id).attr('style') != 'free')
            $('#' + this.id).attr('style', 'max-width: 90px;');
        if ($('[for="' + this.id + '"]').html() == undefined || $('[for="' + this.id + '"]').html() == '')
            $('#' + this.id).attr('style', 'max-width: 90px;');
        $(this).datepicker({
            uiLibrary: 'bootstrap5',
            format: _format,
    ...
    $('[format^="hh"][format$="mm"]').each(function (e) {
        ...
        //console.log(_value);
        if (_value == 'now' || _value == undefined) {
            $(this).val(xDate.Now('HH:mm'));
        }
        else {
            $(this).val(_value);
        }
    ...
    $('span.input-group-addon').on('input', function (e) {
        console.log('hover');
    });
    $('input[format=yyyy]').each(function (e) {
        $('label[for=' + this.id + ']').prependTo($(this).parent());
    });
    $('input[format=yyyy-mm]').each(function (e) {
        $('label[for=' + this.id + ']').prependTo($(this).parent());
    });
    $('input[format=yyyy-mm-dd]').each(function (e) {
        $('label[for=' + this.id + ']').prependTo($(this).parent());
    });
    $("input[default=now]").each(function (e) {
        $('label[for=' + this.id + ']').prependTo($(this).parent());
    });
});
