...
namespace KANBAN.Services.SpecialOrdering.Repository
{
    public class KBNOR250 : IKBNOR250
    {
        ...
        private async Task CheckDataMaster(string SurveyDoc)
        {
            try
            {
                await _kbContext.Database.ExecuteSqlRawAsync("Delete From TB_PDS_Error Where F_User_Name = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value + "'");
                ...
                string _sql = $@"Insert into TB_PDS_Error Select H.F_Survey_Doc,H.F_Supplier_CD,H.F_Supplier_Plant,
                    ...
                    '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}' As F_User_Name, getdate() As F_Create_Date
                    From TB_Survey_Header H inner join TB_Survey_Detail D
                    on H.F_Survey_Doc = D.F_Survey_Doc
                    and H.F_Revise_Rev = D.F_Revise_Rev
                    ...
                    and  H.F_Factory_Code in ('{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value}') and Isnull(T.F_Supplier_Code,'') = ''";
        ...
        public async Task Generate(List<VM_KBNOR250> listObj, [FromQuery] string DeliYM)
        {
            try
            {
                string userName = await _FillDT.GetUserName(_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value);
                userName = userName.Trim().Substring(0, 10);
                string PDSNo = "";
                string FormatPDS = "";
                string FacCD = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value == "1" ? "9Z" : _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value == "2" ? "8Z" : "7Z";
                string _sql = $@"Update TB_Survey_Detail Set F_PDS_No = '', F_PDS_Flg = '0'
                    ...
                    Where F_Plant  in ('{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value}')   and H.F_PDS_Flg = '1' ";
                ...
                _sql = $@"Delete From TB_PDS_TMP Where F_Process_By = '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'";
                ...
                int checkError = _kbContext.Database.ExecuteSqlRaw("Select * From TB_PDS_Error Where F_User_Name = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value + "'");
                ...
                foreach (var obj in listObj)
                {
                    ...
                    DataTable DT = GetSurveyNoPDS(_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value, obj.F_Survey_Doc, DeliveryDT);
                    ...
                    if (PlantType == "B" || StoreCD == "1M" || StoreCD == "1N")
                    {
                        int maxRegRunning = _specialLibs.GetMaxRegRunning(PlantType
                                ...
                                , _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value
                                , DeliYM
                        ...
                        if (_specialLibs.ChkPDSData(PlantType, _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value,
                            DT.Rows[0]["F_Delivery_YM"].ToString().Trim(),
                            DT.Rows[0]["F_Delivery_Trip"].ToString().Trim(), StoreCD) == false)
                        {
                            PDSNo = _specialLibs.GetPDSNo(PlantType, _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value,
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (_specialLibs.ChkPDSData(PlantType, _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value, DT.Rows[0]["F_Delivery_YM"].ToString().Trim(),
                        ...
                        string Cycle = await _specialLibs.getCycleTime(_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value, Supp_CD, Supp_Plant, DeliveryDateFull, DeliveryTrip);
                        ...
                        string Vat = _specialLibs.getVat(_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value);
                        ...
                        _sql = $@"Insert into TB_PDS_Tmp(F_ID, F_Supplier_Code, F_Survey_Doc, F_Delivery_Date, F_PDS_NO, F_Process_By, F_Process_Date) Values
                            ({index + 1}, '{Supp_CD}', '{obj.F_Survey_Doc}', '{DeliveryDateFull}', '{PDSNo}', '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}', getdate())";
