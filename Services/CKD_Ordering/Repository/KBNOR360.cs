    ...
    public class KBNOR360 : IKBNOR360
    {
        ...
        public async Task<DataTable> List_Data()
        {
            try
            {
                var sqlParams = new List<SqlParameter>
                {
                    ...
                    //new SqlParameter("@chrProcessShift_CKD", "D")
                };
        ...
        public async Task Register(List<VM_KBNOR360_Register> listObj)
        {
            ...
            try
            {
                ...
                foreach (var obj in listObj)
                {
                    ...
                    _sql = $@"Update TB_Calculate_V_CKD Set F_Lock= '2',F_Update_By='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'
                    ...
                    _sql = $@"Update TB_Calculate_H_CKD set F_Lock='2',F_Update_By='${_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}' ,F_Update_Date = getdate()
                ...
                _sql = $@"UPDATE TB_Transaction set F_Reg_Flg='3' WHERE  F_OrderType ='N'
                    AND F_Reg_Flg = '2' AND F_Plant='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value}'
                ...
                _sql = $@"DELETE FROM  TB_ORDER_CKD
                    ...
                    and F_FOr_SHift ='{_httpContextAccessor.HttpContext.Request.Headers.FirstOrDefault(x => x.Key == "Shift").Value.ToString()}'";

                await _kbContext.Database.ExecuteSqlRawAsync(_sql);
                _log.WriteLogMsg($"Delete TB_ORDER_CKD : {_sql}");

                _sql = $@"UPDATE TB_Kanban_CUT SET F_KB_Remain = F_KB_Remain + ORD.F_KB_CUT, F_Update_Date=Getdate(),F_Update_By ='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}',
                ...
                _sql = $@"UPDATE TB_Kanban_ADD SET F_KB_Remain = F_KB_Remain - ORD.F_KB_ADD, F_Update_Date=Getdate(),F_Update_By ='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}',
                ...
                _sql = $@"UPDATE TB_Kanban_Stop set F_Update_Date=Getdate(),F_Update_By ='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'
                    ,F_Status = '2'
                ...
                _sql = $@"UPDATE TB_Kanban_Chg_Qty set F_Update_Date=Getdate(),F_Update_By ='{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'
                    ,F_Status = '2'
                    ,F_Start_Date = case when F_Status ='1' then '{KBNOR310.dateProcessDate_CKD.ToString("yyyyMMdd")}' else F_Start_Date end
                    ,F_Start_Shift = case when F_Status ='1' then '{KBNOR310.chrProcessShift_CKD}' else F_Start_Shift end
            ...
            catch (Exception ex)
            {
                ...
                _sql = $@"UPDATE TB_Transaction set F_Reg_Flg='2' WHERE  F_OrderType ='N'
                    AND F_Reg_Flg = '3' AND F_Plant= '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.Locality).Value}'
                    and F_Process_Date ='{KBNOR310.dateProcessDate_CKD.ToString("yyyyMMdd")}'
                    and F_Process_Shift ='{KBNOR310.chrProcessShift_CKD}'
                    ";
        ...
        public async Task<List<string>> GeneratePicking_Click()
        {
            try
            {
                ...
                _sql = $@"UPDATE TB_MS_Parameter SET F_Value2 = '0', F_Value3 = '', F_Update_By = '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'
                    , F_Update_Date = getdate() WHERE F_Code = 'CI_CKD';
                    UPDATE TB_MS_Parameter SET F_Value3 = '{KBNOR310.dateProcessDate_CKD.ToString("yyyyMMdd")}{KBNOR310.chrProcessShift_CKD}', F_Update_By = '{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'
            ...
            try
            {
                ...
                if(DT_TempShelf.Rows.Count > 0)
                {
                    ...
                    await _CKDContext.Database.ExecuteSqlRawAsync($@"EXEC SP_CKDPI_001_New_CancelPicking
                        '{PI_Date_RemainShelf}','{PI_Time_RemainShelf}','{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'");
                    ...
                    {
                        await _kbContext.Database.ExecuteSqlRawAsync($"EXEC [CKD_Inhouse].[sp_RegisterPDS_UT] '{PI_Date}','{PI_Time}','{KBNOR310.dateProcessDate_CKD.ToString("yyyyMMdd")}','{KBNOR310.chrProcessShift_CKD}','{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'");
                    }
            ...
            try
            {
                ...
                if(DT_TempShelf.Rows.Count > 0)
                {
                    await _CKDUSAContext.Database.ExecuteSqlRawAsync($@"EXEC SP_CKDPI_001_New_CancelPicking
                        '{PI_Date}','{PI_Time}','{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'");
                    ...
                    {
                        await _kbContext.Database.ExecuteSqlRawAsync($"EXEC [CKD_Inhouse].[sp_RegisterPDS_UT_USA] '{PI_Date}','{PI_Time}','{KBNOR310.dateProcessDate_CKD.ToString("yyyyMMdd")}','{KBNOR310.chrProcessShift_CKD}','{_httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value}'");
                    }
