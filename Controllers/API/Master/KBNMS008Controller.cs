...
namespace HINOSystem.Controllers.API.Master
{
    ...
    public class KBNMS008Controller : ControllerBase
    {
        ...
        [HttpGet]
        public IActionResult SelectDateChange(string? supplier, string? kanban, string? store, string? partNo, string? selDate)
        {
            try
            {
                ...
                var cycle = _KB3Context.TB_MS_DeliveryTime.Where(x => x.F_Supplier_Code == supplier.Substring(0, 4) &&
                           x.F_Supplier_Plant == supplier[5].ToString() && x.F_Start_Date.CompareTo(selDate) <= 0 && x.F_End_Date.CompareTo(selDate) >= 0
                            ).AsQueryable();
                ...
                string sql = $"SELECT CONVERT(Integer, F_workCd_D{selDate.Substring(6, 2)} " +
                    $") + CONVERT(Integer, F_workCd_N{selDate.Substring(6, 2)}) AS F_Work " +
                    $"FROM TB_Calendar WHERE F_Store_cd = '{storecode}' AND F_YM = '{selDate.Substring(0, 4)}' ";
        ...
        public async Task Insert_TMP(TMP_Planning_Order obj,string selDate)
        {
            try
            {
                ...
                for (int i = 0; i < DT_PartControl.Rows.Count; i++)
                {
                    var TMP_Planning = _KB3Context.TMP_Planning_Order.Where(x => x.F_Supplier_Code == DT_PartControl.Rows[i]["F_Supplier_Cd"].ToString().Trim() &&
                            ...
                            x.F_Delivery_Date == selDate && x.F_Plant == _BearerClass.Plant);
                    ...
                    sql = $"INSERT INTO TMP_Planning_Order(F_Plant, F_Delivery_Date, F_Supplier_Code, F_Supplier_Plant " +
                        ...
                        $", '{selDate}' AS F_Delivery_Date  , RTRIM(C.F_supplier_cd) AS F_Supplier_Code " +
        ...
        public async Task Detail_Data()
        {
            try
            {
                ...
                string dateDelivery = DateTime.Now.ToString("yyyyMMdd");
