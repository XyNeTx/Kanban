...
{
    public class KBNMS011Controller : Controller
    {
            ...
            try
            {
                ...
                _SQL = @"
                    SELECT '' AS RunningNo
                        ...
                        ,F_Part_Name
                        ,F_Store_Cd
	                    , FORMAT(CAST(F_Start_Date AS DATE), 'yyyy-MM-dd') AS F_Start_Date
	                    , FORMAT(CAST(F_End_Date AS DATE), 'yyyy-MM-dd') AS F_End_Date
                        ,F_Update_By
                        ,F_Update_Date
                    FROM dbo.TB_MS_OldPart
                    WHERE 1=1
                ";

                if ( _json != null ) _SQL += " AND F_Plant = "+_json.F_Plant;
        ...
        public IActionResult save()
        {
            ...
            try
            {
                ...
                _TB_MS_OldPart.F_Plant = User.FindFirst(ClaimTypes.Locality).Value;
                ...
                _TB_MS_OldPart.F_Update_By = User.FindFirst(ClaimTypes.UserData).Value.ToString();
