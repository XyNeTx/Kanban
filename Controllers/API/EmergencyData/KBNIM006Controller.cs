...
{
    public class KBNIM006Controller : Controller
    {
        ...
        public IActionResult Import(string fileName = null)
        {
            ...
            try
            {
                ...
                _SQL = @" 
    Delete From TB_Import_EKanban Where F_Update_BY='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_TYpe ='EKanban'; 

    Delete From TB_Import_Error Where F_Update_BY='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_TYpe ='D_EKanban'; 
    
    Delete From TB_Import_Error Where F_Update_BY='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_TYpe ='EKanban'; 
    ...
    ";
                ...
                using (var _stream = new StreamReader(fullPath))
                {
                    while (!_stream.EndOfStream)
                    {
                        ...
                        _SQL = _SQL + "'" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + "', GETDATE())";
    ...
    VALUES('EKanban','" + _PO + "','" + _Item + "','" + _PartNo + "','PART NOT FOUND IN MASTER','" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + "', GETDATE()) ";
                                _KBCN.Execute(_SQL);
    ...
    AND E.F_Update_By='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
";  
                _KBCN.Execute(_SQL);
...
";
                ...
                _SQL = "EXEC dbo.SP_IM003_FilterData '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + "' ";
                _KBCN.Execute(_SQL);
    ...
    WHERE 1=1
    AND F_update_BY='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
    ...
    HAVING COUNT(*) > 1";
                ...
                _SQL = @"
    INSERT INTO TB_IMPORT_ERROR(F_TYpe,F_PDS_CD, F_Row, F_Field, F_Remark, F_Update_By, F_Update_Date)
    SELECT 'EKanban',S.F_PDS_NO,S.F_ITEM_NO,S.F_PART_NO AS F_Part_NO,'Not Found Data IN Parent Child','" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"',GETDATE()
    FROM TB_IMPORT_EKANBAN S 
        LEFT OUTER JOIN(
                SELECT * 
                FROM TMP_Parents_Child C 
                WHERE C.F_Update_By  = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
			    ...
			    AND F_TC_Str COLLATE Thai_CI_AS <= (
												    ...
												    WHERE F_Update_By = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
												    AND F_TYpe = 'EKanban'
    ...
    WHERE S.F_Update_By = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
    AND F_TYpe = 'EKanban' 
...
";
	...
	FROM (
        ...
        , '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' as F_Update_By
    ...
    WHERE I.F_Update_By='" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' 
    ...
    '' as F_Part_Refer,'' as F_Ruibetsu_Refer,'" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' as F_Update_By,getdate() as F_Update_Date,'' as F_Remark
    ...
    from(Select F_PDS_NO, F_part_no, F_Collect_Date, F_Collect_Time, Sum(F_Qty) as F_QTY, substring(F_PDS_NO, 1, 2) as F_PDS_SUB From TB_Import_EKanban Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_TYpe = 'EKanban'
    Sql = Sql & Group by F_PDS_NO, F_part_no, F_Collect_Date, F_Collect_Time, substring(F_PDS_NO, 1, 2)
    Sql = Sql & )S INNER JOIN(Select * from dbo.TMP_PARENTS_CHILD Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"')P ON
    ...
    (Select F_Part_NO, F_Ruibetsu, F_Store_Cd, F_TC_Str, F_TC_End, F_Supplier_Cd, F_Plant, F_Sebango, F_Ratio, F_Part_Nm, F_QTY_BOX from TMP_CONSTRUCTION Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and  F_Supplier_CD<>'9997')C
    LEFT OUTER JOIN(Select F_part_no, F_RUibetsu, F_Store_CD, F_Ratio_N from dbo.TMP_PRG006_parameter Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"') P
    ...
    )S INNER JOIN(Select * from dbo.TMP_PARENTS_CHILD Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"')P ON
    substring(S.F_PART_NO, 1, 5) + substring(S.F_PART_NO, 7, 5) = rtrim(P.F_PARENT_PART) collate Thai_CI_AS
    AND substring(S.F_PART_NO,13,2) = P.F_ruibetsu collate Thai_CI_AS
    ...
    LEFT OUTER JOIN(select C.F_Part_No, C.F_Ruibetsu, C.F_Store_CD, C.F_TC_Str, C.F_TC_End, C.F_Supplier_Cd, C.F_Plant, C.F_Sebango, C.F_QTY_BOX, C.F_Ratio, C.F_Part_NM, P.F_Ratio_N from
    (Select F_Part_NO, F_Ruibetsu, F_Store_Cd, F_TC_Str, F_TC_End, F_Supplier_Cd, F_Plant, F_Sebango, F_Ratio, F_Part_Nm, F_QTY_BOX from TMP_CONSTRUCTION Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_Supplier_CD<>'9997')C
    LEFT OUTER JOIN(Select F_part_no, F_RUibetsu, F_Store_CD, F_Ratio_N from TMP_PRG006_parameter WHERE F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"') P
    ON C.F_Part_no = P.F_Part_no and C.F_Ruibetsu = P.F_Ruibetsu and C.F_Store_Cd = P.F_Store_Cd)CON
    ON P.F_Child_Part = CON.F_Part_NO collate Thai_CI_AS
    and P.F_Ch_Ruibetsu = Con.F_ruibetsu collate Thai_CI_AS
    and P.F_Ch_Store_Cd = Con.F_Store_CD collate Thai_CI_AS
    and substring(S.F_Collect_Date,7,4)+substring(S.F_Collect_Date, 4, 2) + substring(S.F_Collect_Date, 1, 2) >= Con.F_TC_Str collate Thai_CI_AS
    and substring(S.F_Collect_Date,7,4)+substring(S.F_Collect_Date, 4, 2) + substring(S.F_Collect_Date, 1, 2) <= Con.F_TC_End  collate Thai_CI_AS
    LEFT OUTER JOIN(Select * from dbo.TMP_PARENT_PART Where F_Update_BY = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"')PP ON
    P.F_Parent_Part = PP.F_Parent_Part And P.F_Ruibetsu = PP.F_Ruibetsu And P.F_Store_CD = PP.F_Store_CD
    and substring(S.F_Collect_Date,7,4)+substring(S.F_Collect_Date, 4, 2) + substring(S.F_Collect_Date, 1, 2) >= PP.F_TC_Str collate Thai_CI_AS
    and substring(S.F_Collect_Date,7,4)+substring(S.F_Collect_Date, 4, 2) + substring(S.F_Collect_Date, 1, 2) <= PP.F_TC_End collate Thai_CI_AS
    LEFT OUTER JOIN(Select * from TB_Transaction_TMP Where F_Update_By = '" + _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.UserData).Value.ToString() + @"' and F_Type = 'EKanban' and F_Plant = '" + @"')TMP
