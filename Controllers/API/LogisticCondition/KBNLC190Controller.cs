...
{
    public class KBNLC190Controller : Controller
    {
        ...
        public IActionResult Period([FromBody] string pData = null)
        {
            ...
            try
            {
                ...
                string _jsImport_Delivery = _KB3Connect.executeSQLJSON(_SQL, pUser: _JBearer, pControllerName : ControllerContext.ActionDescriptor.ControllerName, pActionName: ControllerContext.ActionDescriptor.ActionName);
        ...
        public IActionResult Search([FromBody] string pData = null)
        {
            ...
            try
            {
                ...
                _SQL = @" EXEC [exec].[spKBNLC190_SEARCH] '" 
                    + _json.Plant + @"', '" 
                    + _json.Period + @"', '" 
                    + _json.Rev + @"', '" 
                    + _json.Flag + @"' ";
        ...
        [HttpPost]
        public IActionResult GetDateStartForecast([FromBody] string pData = null)
        {
            dynamic _json = null;
            string _SQL = "";
            try
            {
                ...
                _SQL = @" 
                    SELECT LEFT(StartForecastDate,4) 
	                    + '-' + SUBSTRING(StartForecastDate,5,2)
	                    + '-' + RIGHT(StartForecastDate,2) AS ForecastDate
	                    ,StartForecastDate AS StartForecastDate
                    FROM (
	                    SELECT dbo.FN_GetDateStartForecast('" + _json.StartDate + @"',3) AS StartForecastDate
	                    ) dt
                    ";
                string _nCheckDate = _KB3Connect.executeSQLJSON(_SQL, pUser: _JBearer, pControllerName : ControllerContext.ActionDescriptor.ControllerName, pActionName: ControllerContext.ActionDescriptor.ActionName);
